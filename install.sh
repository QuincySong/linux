...
Lock_Clear(){
	if [ -f "/etc/bt_crack.pl" ];then
		...
		\cp -rpa /Quincy/backup/panel/vhost/* /Quincy/server/panel/vhost/
		mv /Quincy/server/panel/BTPanel/__init__.bak /Quincy/server/panel/BTPanel/__init__.py
	...
	do
		if [ -f "/etc/init.d/${serviceS}" ]; then
			if [ "${serviceS}" = "httpd" ]; then
				serviceCheck=$(cat /etc/init.d/${serviceS}|grep /Quincy/server/apache)
			elif [ "${serviceS}" = "mysqld" ]; then
				serviceCheck=$(cat /etc/init.d/${serviceS}|grep /Quincy/server/mysql)
			else
				serviceCheck=$(cat /etc/init.d/${serviceS}|grep /Quincy/server/${serviceS})
			fi
...
Install_Bt(){
	...
	mkdir -p /Quincy/server
	mkdir -p /Quincy/web
	mkdir -p /Quincy/logs
	mkdir -p /Quincy/backup/database
	mkdir -p /Quincy/backup/site
	...
	wget -O /Quincy/server/panel/install/public.sh http://download.bt.cn/install/public.sh -T 10
...
Set_Bt_Panel(){
	...
	sleep 1
	admin_auth="/Quincy/server/panel/data/admin_path.pl"
...
Setup_Count(){
	...
	if [ "$1" != "" ];then
		echo $1 > /Quincy/server/panel/data/o.pl
		cd /Quincy/server/panel
		python tools.py o
	fi
	echo /Quincy > /var/bt_setupPath.conf
}
